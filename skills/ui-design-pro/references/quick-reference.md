...
Load this file when doing a UI review/audit pass, or when you need the full checklist for a category beyond the Review Priority table in SKILL.md. Most rules are also in `data/ux-guidelines.csv` and reachable via `--domain ux` — this file is a static index for scanning without a search round-trip.

**Scope note.** This skill is web-only, but the rule set below is drawn from the Apple HIG and Material Design and keeps its platform-specific entries. Anything naming a native affordance — safe areas, notch/Dynamic Island, haptics, Dynamic Type, VoiceOver, iOS Tab Bar, Android Top App Bar, predictive back — does not apply to a web target. Read those as background on *why* a rule exists, and apply the web equivalent: `prefers-reduced-motion` for Reduced Motion, `rem`-based type scales for Dynamic Type, `aria-*` for accessibility labels, sticky-element insets for safe areas.
