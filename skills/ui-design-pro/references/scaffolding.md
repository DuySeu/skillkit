...
Read this at **step 11**, and only on a greenfield project. On a project that already
has a `package.json` there is nothing here for you: skip to step 12.
...
`docs/design/` preserved beside it and the dotfiles (`.gitignore`) intact.
...
```css
/* src/index.css — replace the template's contents entirely */
@import "tailwindcss";
```
...
at step 11, because the framework was confirmed at step 2 and the direction was picked
at step 9. It is a Red Flag at any earlier point. On a fresh Vite app it also needs the
