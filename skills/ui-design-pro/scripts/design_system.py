...
Usage:
    ...
    print(result["text"])
    result["design_system"]  # raw dict, for --json callers

This returns one seed recommendation for a query. It writes nothing to disk --
persisting a design system as generated markdown was removed deliberately: the
token file (index.css or the framework's theme config) is the source of truth,
and a generated copy of it only ever drifted.
"""
...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None,
        ...
        return {
            ...
            # Full colors.csv row, so the reported token set is the one that
            # actually gets implemented. Do not drop card/muted-foreground:
            # muted-foreground is the pair that fails the contrast gate most
            # often, so the implementer needs to see its value up front.
            "colors": {
...
def format_ascii_box(design_system: dict) -> str:
    ...
    color_entries = [
        # shadcn token names -- these are what framework-recipes.md maps from,
        # what contrast-check.mjs parses, and what mockup-template.html uses.
        # A "--color-primary" here would document a variable no code declares.
        ("Primary",             "primary",         "--primary"),
        ("Primary Foreground",  "on_primary",      "--primary-foreground"),
        ("Secondary",           "secondary",       "--secondary"),
        ("Secondary Foreground","on_secondary",    "--secondary-foreground"),
        ("Accent/CTA",          "accent",          "--accent"),
        ("Accent Foreground",   "on_accent",       "--accent-foreground"),
        ("Background",          "background",      "--background"),
        ("Foreground",          "foreground",      "--foreground"),
        ("Card",                "card",            "--card"),
        ("Card Foreground",     "card_foreground", "--card-foreground"),
        ("Muted",               "muted",           "--muted"),
        ("Muted Foreground",    "muted_foreground","--muted-foreground"),
        ("Border",              "border",          "--border"),
        # shadcn declares --input separately but stock themes give it the same
        # value as --border; documented as its own row so the implementation
        # doesn't silently leave --input undefined.
        ("Input",               "border",          "--input"),
        ("Destructive",         "destructive",     "--destructive"),
        ("Destructive Fg",      "on_destructive",  "--destructive-foreground"),
        ("Ring",                "ring",            "--ring"),
    ]
...
def generate_design_system(query: str, project_name: str = None, output_format: str = "markdown",
    ...
    Returns:
        dict with keys: "text" (formatted design system string) and
...
if __name__ == "__main__":
    ...
    parser.add_argument("--format", "-f", choices=["ascii", "markdown"], default="markdown",
                        help="Output format (default: markdown; 'ascii' draws a box for a terminal)")
