#!/usr/bin/env python3
"""Fill proposal DOCX from approved Markdown using the bundled VN template."""
...
def fill_scope_table(table, rows: List[List[str]]) -> None:
    """Fill IN/OUT scope table (two columns, one data row with bullet lists)."""
    if len(rows) < 2 or len(table.rows) < 2:
        return
    in_lines: List[str] = []
    out_lines: List[str] = []
    for row in rows[1:]:
        if len(row) >= 1 and row[0].strip():
            in_lines.append(row[0].strip())
        if len(row) >= 2 and row[1].strip():
            out_lines.append(row[1].strip())
...
def set_paragraph_text(paragraph: Paragraph, text: str) -> None:
    """Replace all text in a paragraph with a single string."""
    for run in paragraph.runs:
        run.text = ""
    if paragraph.runs:
...
def apply_cover(doc: Document, frontmatter: Dict[str, str]) -> None:
    """Set cover lines from frontmatter (proposal template indices 0, 2–4)."""
    cover_map = [
        ("title", 0),
        ("prepared_by", 2),
        ("date", 3),
...
def to_doc_heading(md_title: str) -> str:
    """Convert markdown subsection title to template heading (single space)."""
...
def apply_section_headings(doc: Document, titles: Dict[str, str]) -> None:
...
def fill_tables_from_sections(doc: Document, sections: Dict[str, str]) -> None:
    """Map parsed markdown tables to fixed proposal template table indices."""
    if "1.2" in sections:
    ...
    if "1.3" in sections:
        tables = parse_markdown_tables(sections["1.3"])
        if tables and len(doc.tables) > 1:
            fill_scope_table(doc.tables[1], tables[0])

    mapping_keys = sorted(
        key
        for key in sections
        if re.match(r"^3\.\d+$", key) and key != "3.3"
    )
    for offset, key in enumerate(mapping_keys[:2]):
    ...
    if "4" in sections:
        tables = parse_markdown_tables(sections["4"])
        if tables and len(doc.tables) > 4:
            fill_table(doc.tables[4], tables[0])

    if "5" in sections:
        tables = parse_markdown_tables(sections["5"])
        if tables and len(doc.tables) > 5:
            fill_table(doc.tables[5], tables[0])

    if "6" in sections:
        tables = parse_markdown_tables(sections["6"])
        if tables and len(doc.tables) > 6:
...
def main() -> None:
    ...
    default_template = skill_root / "assets" / "proposal-template.docx"

    parser = argparse.ArgumentParser(
        description="Export proposal Markdown to styled DOCX."
    )
    parser.add_argument("markdown", type=Path, help="Approved proposal markdown path")
    parser.add_argument(
        "--template",
        type=Path,
