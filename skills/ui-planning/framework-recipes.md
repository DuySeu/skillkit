...
>
> On a **greenfield** guide, header says `Stack: not chosen yet`. Whoever scaffolds later reconciles wiring against the majors actually installed.
...
This skill stops at the guide. Author mode writes no app code. Everything below applies in **Comply** when an app exists: read majors from `package.json`, then map shadcn-vocabulary tokens into that engine.

If you are scaffolding an app in a separate session, do not use `--overwrite` on a folder that already holds `docs/` (it can wipe the guide and option previews).
