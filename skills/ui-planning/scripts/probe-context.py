...
Step 1 of the authoring flow. It answers the questions that are *facts about
files* -- which framework, which major, which UI library, is there already a
token file, did a previous run leave a design guide -- so the interview only
spends the user's attention on what genuinely isn't written down anywhere.

The single most consequential thing it reports is whether `docs/DESIGN.md`
already exists, because that decides which MODE the skill runs in: a guide on
disk means comply with it, not author a new one over the top of it.
...
# Meta-framework → base FE (Nuxt 3 reports as Vue 3 for UI-library detection).
FE_BASE = {
    "Next.js": "react",
    "Nuxt 3": "vue",
    "SvelteKit": "svelte",
...
def scan_css(root, limit=60):
    ...
    All of docs/ is excluded on purpose. Everything this skill writes there is
    full of shadcn tokens -- the rejected options under docs/design/, and the
    generated docs/index.css itself -- so counting them would make every second
    run report "this project already has a theme" on the strength of the skill's
    own output. They are reported under prior design work instead. THEMED means
    the *app's* stylesheet carries decisions, which is a different fact.
    """
    out = []
...
def probe(root):
    ...
    if pkg is None and not composer:
        # A finished Author run on greenfield still has no package.json, but
        # docs/DESIGN.md means Comply — do not re-author over it.
        r["design_docs"] = find_design_docs(root)
        r["verdict"] = "GUIDED" if r["design_docs"].get("guide") else "GREENFIELD"
        return r
...
def find_design_docs(root):
    """What a previous run of this skill, or of a related one, already left behind.

    `guide` is the load-bearing key -- it is what flips the verdict to GUIDED and
    therefore what puts the skill in comply mode instead of authoring mode.
    """
    docs = root / "docs"
    d = docs / "design"
    legacy = sorted(str(p.relative_to(root)) for p in root.glob("design-system/*/MASTER.md"))
    return {
        "guide": "docs/DESIGN.md" if (docs / "DESIGN.md").exists() else None,
        "guide_css": "docs/index.css" if (docs / "index.css").exists() else None,
...
NEXT_STEPS = {
    "GREENFIELD": """\
  MODE: author.
  1. Say in one line that there is no project here, then create
     ./<project-slug>/docs/ and NOTHING else. No package.json, no src/.
  2. Do NOT ask about the stack. Record "Stack: not chosen yet" in the
     guide header later.
  3. Ask the two concept questions: (1) product/who uses it; (2) pick one
     anchor -- brand colour, OR inspiration site, OR neither.
  4. Invent 3-5 options by hand into option-tokens/ + manifest.json.""",
    "FRESH": """\
  MODE: author.
  1. Report what you found above in one short paragraph.
  2. Do NOT ask about the stack. Copy detected versions into the guide header
     as context later -- detected is not a question and nothing waits on it.
  3. Ask the two concept questions (product; then colour OR inspiration OR
     neither). No refresh-vs-replacement -- there is no existing theme.
  4. Invent 3-5 options by hand into option-tokens/ + manifest.json.""",
    "THEMED": """\
  MODE: author, over a project that already has visual decisions in it.
  ...
  2. ASK THE USER: refresh or replacement?
       refresh     -- keep the token NAMES and the component structure, change
                      the values. Options are authored to fit what exists,
                      and the guide's component recipes describe what is there.
     ...
     gets authored, so it has to be answered before the options are written.
  3. Read the screen files listed above for density and layout conventions
     ...
     What survives goes into the guide's Layout section as a recorded rule.
  4. Do NOT ask about the stack. Record detected versions in the guide header.
  5. Invent 3-5 options by hand into option-tokens/ + manifest.json.""",
    "GUIDED": """\
  MODE: comply. A design guide already exists -- do NOT author a second one.
  1. READ docs/DESIGN.md in full, and docs/index.css if it is listed
     above. They are the design decisions for this project; you did not make
     them and this run does not get to remake them.
  2. Then split on what the user actually asked for:
       "build / style / fix some UI"  -- build it to the guide. Tokens from
                      docs/index.css (copy it to the app's real stylesheet if
                      that has not happened yet), surface kit, motion, and
                      anti-patterns all apply. Do not re-author options and do
                      not re-run the interview.
       "change the design direction"  -- the guide is being revised, not
                      replaced. Say which sections you are about to change,
                      get a yes, then edit IN PLACE. Never a v2 file.
  3. If the guide is silent on something you need, that is a gap in the guide:
     decide the smallest thing consistent with the direction, do the work, and
     say in the chat what you decided so it can be written into the guide.
  4. If the guide contradicts the stack above (a Tailwind major that moved, a
     library that was swapped), say so before building. A guide written against
     a different major is the one thing worth stopping for.""",
}
...
def report(r):
    ...
    if r["verdict"] == "GREENFIELD":
        ...
        a("PRIOR DESIGN WORK")
        found = False
        for key, label in (("guide", "DESIGN GUIDE"), ("guide_css", "guide tokens"),
                          ("ui_plan", "UI-PLAN.md")):
            if docs.get(key):
    ...
    a("")
    a("None of the above is a confirmation. State detected versions back as")
    a("context; do not interview the stack in Author mode.")
    return "\n".join(out)
